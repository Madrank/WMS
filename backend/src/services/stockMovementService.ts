...
export const stockMovementService = {
  async list({ articleId, locationId, type, page = 1, limit = 20 }: { articleId?: number; locationId?: number; type?: string; page?: number; limit?: number }) {
    const { rows, total } = await stockMovementRepository.findAll({ articleId, locationId, type, page, limit });
    return {
      data: rows,
